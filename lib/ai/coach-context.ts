...
export async function loadCoachInsights(accountId?: string) {
  ...
  if (error || !data) {
    return "Trade stats could not be loaded. Coach from process and the current page only.";
  }

  const trades = data.map((row) => normalizeTrade(tradeFromRow(row as TradeRow)));
  if (trades.length === 0) {
  ...
  const recent = trades.slice(0, 8).map((trade) => {
    const pnl = trade.pnlDollars ?? 0;
    const signed = pnl > 0 ? `+${money(pnl)}` : money(pnl);
    return `${trade.pair} ${trade.direction} ${trade.outcome} ${signed}${
      trade.strategy ? ` #${trade.strategy.replace(/^#/, "")}` : ""
    }`;
  });
  ...
  return [
    `Trades: ${summary.tradeCount}. Wins ${summary.wins}, losses ${summary.losses}, BE ${summary.breakevens}.`,
    `Win rate: ${winRate}%. Net P/L: ${
      summary.totalNetProfit > 0 ? "+" : ""
    }${money(summary.totalNetProfit)}.`,
    `Max drawdown: ${money(summary.maxDrawdown)}.`,
    strategies.length
      ? `Top setups: ${strategies
          .map(
            (item) =>
              `${item.name} (${item.trades} trades, ${Math.round(item.winRate)}% WR, ${
                item.netPnl > 0 ? "+" : ""
              }${money(item.netPnl)})`
          )
          .join("; ")}.`
