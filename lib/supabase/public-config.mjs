...
export const supabasePublicConfig = {
  ...
  siteUrl: "https://edge-log-11.netlify.app",
};
...
export function resolveSiteUrl() {
  const production = supabasePublicConfig.siteUrl.replace(/\/$/, "");

  const cleaned = sanitizeEnvValue(process.env.NEXT_PUBLIC_SITE_URL).replace(
    /\/$/,
    ""
  );
  ...
  if (cleaned && !isLocalHost(cleaned)) {
    const normalized = cleaned.startsWith("http")
      ...
      : `https://${cleaned}`;
    try {
      if (new URL(normalized).origin === new URL(production).origin) {
        return production;
      }
    } catch {
      return production;
